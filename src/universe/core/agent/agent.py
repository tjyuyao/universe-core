...
from ..llm_client import LLMClient, LLMResult, estimate_tokens, BudgetWarning, ToolCall, recover_action_from_tool_call
...
class Agent(Object):
    ...
    def _build_system_prompt(self, world: World) -> str:
        """构建系统提示"""
        return f"""用户消息中包含最新的完整上下文。你需要根据上下文，深度扮演符合接下来描述的角色的人格，具体是指你需要做出符合角色性格、能力、情绪、思维模式的工具调用来与世界交互。你必须在一次回复中调用所有必要的工具，工具调用没有回显，但会影响你下一次的观测，同理，你现在看到的是上一次行为影响后的结果。我们仅使用单条用户消息来包含完整的上下文，而不使用多轮对话模型，它模拟了你的整个可观状态空间，工具调用模拟了你的整个可用动作空间。只有工具调用是必要的输出，输出的 content 没有任何额外效用；但对于困难的问题，在 content 中包含临时的中间思考过程是适宜的；但长思考会影响动作发出的时延，因此如果上下文情境处于紧急场合，则不应过度思考。角色的描述如下：
...
{context_str}""")
    ...
    def _parse_response(self, response: LLMResult, world: World) -> dict[str, list[ToolCall]]:
        ...
        for tool_call in response.tool_calls:
            ...
            # 处理某些模型返回格式不正确的工具调用
            # 示例1：{'arguments': {'i_have_reasoned_that': 'reasoning...'}, 'name': ''}
            # 示例2：{'arguments': {'': 'i_have_reasoned_that', 'reasoning': '...'}, 'name': ''}
            if not action_name and arguments:
                action_name, arguments = recover_action_from_tool_call(
